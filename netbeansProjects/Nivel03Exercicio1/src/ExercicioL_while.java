...
// </editor-fold>
		...
		/*
		Elaborar um programa que leia quinze valores numéricos inteiros e no 
		final apresente o somatório da fatorial de cada valor lido
		*/
		
		
		Integer contNumLido = 1;
		Integer limiteNumLido = 2; //original deve ser 15
		Integer numLido = 0;
		Double somaFatorial = 0.0;
		
		while (contNumLido <= limiteNumLido) {
			numLido = readInteger("Digite o " + contNumLido + "º número");		
			
			//calcular o fatorial atual
			Integer contReversoFatorial = numLido;
			Integer limiteFatorial = 1;
			Integer fatorial = 1;

			if (numLido >= 0) {
				while (contReversoFatorial >= limiteFatorial ) {
					fatorial = fatorial * contReversoFatorial;
		...
		write("A soma de todos os fatoriais são: " + somaFatorial);
