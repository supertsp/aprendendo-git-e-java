...
public class TesteOperadorModulo {
...
// </editor-fold>
		...
		/*
		Tendo-se em mente que toda divisão tem um dividendo (número a ser dividido),
		um divisor (número que divide o dividendo) e um quociente (número resultante da divisão),
		o operador módulo (símbolo de %) não foge dessa regra tendo apenas como diferença o valor do
		quociente que vem a ser o resto da divisão. Agora faça um programa que leia o
		divisor do usuário e exiba o quociente das sucessivas divisões entre a faixa
		numérica de 0 a 100.
		*/
		
		Integer dividendo = 0;
		Integer divisor = readInteger("Digite o valor do divisor");
		Integer quocienteResto = 0;
		Integer fimRepeticao = 100;
		
		while (dividendo <= fimRepeticao) {
			quocienteResto = dividendo % divisor;
			write(dividendo + " % " + divisor + " = " + quocienteResto);
			dividendo++;
		}
		
		dividendo = (int) (Math.random() * 6);
