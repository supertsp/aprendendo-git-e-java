...
// </editor-fold>
		...
		Double a = readDouble("Digite o valor de A para calcular a equação do 2º grau");
		Double b = readDouble("Digite o valor de B para calcular a equação do 2º grau");
		Double c = readDouble("Digite o valor de C para calcular a equação do 2º grau");
		...
		CASO 1
		delta < 0 – não há solução real,
		
		CASO 2
		delta > 0 – há duas soluções reais e diferentes
		
		CASO 3
		delta = 0 – há apenas uma solução real
		*/
		
		Double delta = Math.pow(b, 2) - (4 * a * c);
		
		if (delta < 0) {
			write("não há solução real");
		}
		
		if (delta > 0) {
			write("há duas soluções reais e diferentes");
		...
		if (delta == 0) {
			write("há apenas uma solução real");
